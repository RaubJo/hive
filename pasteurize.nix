...
let
  ...
  evalModulesMinimal = (import (nixpkgs + /nixos/lib/default.nix) {
    inherit (nixpkgs) lib;
  ...
  beeOptions = { config, ... }: {
    options.bee = {
      ...
      pkgs = l.mkOption {
        ...
        apply = x:
          if (l.hasAttr "${config.bee.system}" x) then
            x.${config.bee.system}
          else
            x;
      };
  ...
  combCheckModule = let
    erase = optionName:
      { options, ... }:
      let opt = l.getAttrFromPath optionName options;
      in {
        options = l.setAttrByPath optionName (l.mkOption { visible = false; });
        config._hive_erased = [{
          assertion = !opt.isDefined;
          message = ''
            The option definition `${l.showOption optionName}' in ${
              l.showFiles opt.files
            }  is not supported by divnix/hive.
  ...
  */
  pasteurize = self:
    l.pipe (l.mapAttrs (system:
      l.mapAttrs (user: blocks:
        (l.pipe blocks [
          (l.attrByPath [ cellBlock ] { })
          (l.mapAttrs (machine:
            checkAndTransformConfigFor user machine (asserted: {
  ...
  stir = config: {
    ...
    evalConfig =
      import (config.nixpkgs.pkgs.path + "/nixos/lib/eval-config.nix");
        ...
        (l.pipe blocks [
          ...
          (l.mapAttrs (homecfg:
            checkAndTransformConfigFor user homecfg (
              ...
              # below
              asserted: {
                bee = { inherit (asserted.bee) system pkgs home; };
              })))
  ...
  showAssertions = let
    ...
    showWarnings = res:
      let f = w: x: l.trace "warning: ${w}" x;
      in l.fold f res res.config.warnings;
  in evaled:
  showWarnings (let
